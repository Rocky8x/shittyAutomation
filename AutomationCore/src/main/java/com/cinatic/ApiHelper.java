...
public class ApiHelper {
	...
	public Response userLogin(String username, String password) {
		...
		return oauth(username, password);
	}
	 ...
	 * @return
	 */
	public Response oauth(String username, String password) {
	...
	public class Helper {
		...
		public ArrayList<Device> getDevices() {
			...
			for (Object id : c_response.jsonPath().getList("data.device_id")) {
				devices.add(getDeviceByDeviceId((String) id));
			}
			return devices;
		}

		public List<Object> extractCloudLogFile() {

			return c_response.jsonPath().getList("data.url");
		}

		public String downloadCloudLogFile(String deviceID) {

			Log.info("Download log of device: ", deviceID);
			ApiHelper.this.postRequestUploadLog(deviceID);
			String file = getUploadLogFile();
			for (int i = 0; i < 5; i++) {
				ApiHelper.this.getCloudLogs(deviceID);
				List<Object> logFiles = extractCloudLogFile();
				for (Object object : logFiles) {
					if (((String) object).contains(file)) {
						return downloadFile((String) object, file);
					}
				}
		...
		public String getUploadLogFile() {
			...
			return c_response.jsonPath().getString("data.file_name");
		}

		public String downloadFile(String scr, String des) {

			des = StringHelper.getSystemPath() + File.separatorChar + des;
			Log.info("Download log to: ", des);
			try {
				FileUtils.copyURLToFile(new URL(scr), new File(des));
			} catch (Exception e) {
				Log.fatal("Download file failed: ", e.getMessage());
				return null;
			}
			return des;
		}
