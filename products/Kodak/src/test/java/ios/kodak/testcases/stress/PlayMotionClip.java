...
public class PlayMotionClip extends TestBaseIOS {
	...
	public void testPlayMdClip(String tc_name) throws InterruptedException {
		...
		if (device_status.equals("Online")) {
			...
			Assert.assertEquals(device_mode.contains("OK"), true,
					"Error: actual mode is " + device_mode);
	...
	public void afterMethod(ITestResult result) throws InterruptedException {
		...
		try {
			...
			String	fileName	= tc_name + ("_" + startPlayVideo.replace(":", "-")).toLowerCase()
					+ ".txt";
			...
			db.doInsertVideoData(c_device, clipType, playDuration, startPlayVideo,
					clipReadyDuration + "", driverSetting.getDeviceName(), errorCode, fileUrl,
					c_username + "/" + c_password, "PLAY MOTION CLIP PERFORMANCE");
	...
	public Object[][] sdCardMDClip() {
		...
		return new Object[][] {	{ "sdCardMDClip01" },
								{ "sdCardMDClip02" },
								{ "sdCardMDClip03" },
								{ "sdCardMDClip04" },
								{ "sdCardMDClip05" },
	...
	public Object[][] cloudMDClip() {
		...
		return new Object[][] {	{ "cloudMDClip01" },
								{ "cloudMDClip02" },
								{ "cloudMDClip03" },
								{ "cloudMDClip04" },
								{ "cloudMDClip05" },
