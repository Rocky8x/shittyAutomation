...
// Assert.assertEquals(streamMode.contains("OK"), true, "Error: actual mode is "
// + streamMode);
// Assert.assertTrue(hasStream, "Error: Load streaming is " + hasStream);
	...
	public void afterMethod(ITestResult result) throws InterruptedException {
		...
		try {
			...
			// Currently, haven't found any method to get iphone log but just prepare the
			// file name in case i can get the log from the phone in the future
			String	logFileName	= tc_name + ("_" + startStream.replace(":", "-")).toLowerCase()
					+ ".txt";
			...
			db.doInsertStreamData(device, streamMode, streamingDuration, startStream, appVersion,
					driverSetting.getDeviceName(), errorCode, fileUrl,
					c_username + "/" + c_password, "STREAMING PERFORMANCE");
