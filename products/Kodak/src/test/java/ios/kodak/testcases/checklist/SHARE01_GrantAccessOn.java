...
public class SHARE01_GrantAccessOn extends TestBaseIOS{

	ApiHelper api;
	Device device;
	
	@Test(priority = 37, description = "Verify that user can share camera via with allow access right on")
	public void shareDeviceViaEmailWithAccessRightOn() {
		api = new ApiHelper();
		...
		// Grant access to another user with allow access on
		...
		Assert.assertTrue(PageDeviceSettings.verifySettingTitleDisplay(), "Verify user should not access to Camera setting.");
