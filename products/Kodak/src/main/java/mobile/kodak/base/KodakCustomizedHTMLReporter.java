	...
	private Map<String, String> getHtmlResult() {
		...
		if (files != null) {
			for (File file : files) {
				...
				if (file.toString().matches(".*" + File.separatorChar + "suite\\d+_test\\d+_results.html")) {
...
//		}
	...
	private void modifyOverviewReport() throws Exception {
		...
		while ((line = bufferedReader.readLine()) != null) {
			if (line.contains("<table class=\"overviewTable\">")) {
				bufferedWriter.write("<h2>" + " App version = " + TestConstant.appVersion+ "</h2>");
	...
	private void modifySuitesReport() throws Exception{
		...
		for (String oldResultFile : oldResults) {
			...
			BufferedReader bufferedReader = new BufferedReader(new FileReader(oldResultFile));
			BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(newResultFile));
			String lineRead;
			String lineWrite = "";
			String duration;
			...
			oldFile.renameTo(new File(htmlpath + File.separatorChar + oldFile.getName() + ".backup"));
