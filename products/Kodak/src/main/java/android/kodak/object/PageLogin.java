...
public class PageLogin extends PageBase {
	...
	public static Element getGGPwdNextButton() {
		...
		return new Element(By.xpath(nextBtn))
				.setDescription("Google login: Next button @ password page");
	}
	...
	public static Element getExistedGoogleAccount(String email) {
		...
		return new Element(By.xpath(String.format(emailAccount, email)), 10)
				.setDescription(String.format("Exiting google email: %s", email));
	}

	/**
	 * In the first time login with google, Terms of service and privacy policy page
	 * display
	...
	public static Element getFbLoginButton() {
		...
		String loginBtn = "//android.widget.Button";
	 ...
	 * @return
	 */
	public static Element getFbContinueBtn() {

		String continueBtn = "//android.widget.Button";
		return new Element(By.xpath(continueBtn)).setDescription("FB login: continue");
	}
	 ...
	 * 2. Have some Gmail logged in, but not match with gmail variable --> click
	 * google icon --> click 'Use another account' --> login Gmail form
	 * 3. Have Gmail account login before and contains gmail variable --> click
	 * google icon -- click existed gmail
	 * 
	 * @param email
	 * @param password
	 */
	public static void loginWithGoogleAccount(String email, String password) {
		...
		if (getGoogleAccPanel().getWebElement() != null) {
			...
			inputDataIntoGoogleLoginForm(email, password);
			getIAgreeButton().click();
			if (getGGServiceTitle().getWebElement() != null) { clickGoogleAcceptButton(); }
		}
	...
	public static boolean verifyAccountExisted(String email) {
		...
		boolean	rs		= false;
		int		count	= 1;
		while (count <= 10 && getExistedGoogleAccount(email).getWebElement() == null
				&& getUseAnotherAccountLink().getWebElement() == null) {
			swipeTopToBottom();
			count++;
		}
		if (getExistedGoogleAccount(email).getWebElement() != null) { rs = true; }
		return rs;
	}
	...
	public static String getLoginMessage() {
		...
		if (getLoginMsg().getWebElement() != null) { return getLoginMsg().getText(); }
	...
	public static void loginWithFacebookAccount(String email, String password) {
		...
		Element			contButton	= getFbContinueBtn();
		WebDriverWait	wait		= new WebDriverWait(DriverManager.getDriver().getAppiumDriver(),
				10);
		wait.until(ExpectedConditions.elementToBeClickable(contButton.by));
		contButton.click();
	}
	...
	public static boolean isDisplayed() {
		...
		if (getSignInButton().getWebElement() != null)
			return true;
		return false;
	}
